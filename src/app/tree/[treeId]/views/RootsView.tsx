...
const EditableField = forwardRef<HTMLInputElement | HTMLTextAreaElement, EditableFieldProps>(({ value, onUpdate, placeholder, isMagical, className, asTextarea }, ref) => {
  ...
  const commonProps = {
    ...
    className: cn(
        "w-full bg-slate-800/80 border-2 border-slate-600 rounded-2xl px-4 py-3 text-base text-white text-right placeholder:text-slate-400 placeholder:text-right",
...
const StudentSelector = ({ people, currentStudentId, onStudentChange }: { people: Person[], currentStudentId?: string, onStudentChange: (id: string) => void }) => {
  ...
  return (
    <div className="relative" dir="rtl">
      <button
        ...
        className="w-full h-[48px] flex items-center justify-between px-4 rounded-2xl bg-slate-800/80 border-2 border-teal-500/50 text-white text-base hover:border-teal-400 transition-all duration-200"
      >
        <div className="flex items-center gap-2">
          <BadgeCheck className="h-4 w-4 text-teal-400" />
          {/* chevron */}
          <svg className={cn("h-4 w-4 text-slate-400 transition-transform", isOpen && "rotate-180")} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full mt-2 w-full z-50 bg-slate-800 border border-slate-600 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden"
          >
            <ScrollArea className="max-h-56">
              {people.map(person => (
                <button
                  key={person.id}
                  type="button"
                  onClick={() => { onStudentChange(person.id); setIsOpen(false); }}
                  className={cn(
                    "w-full flex items-center justify-end gap-3 px-4 py-3 text-right hover:bg-slate-700 transition-colors",
                    person.id === currentStudentId && "bg-indigo-900/50"
                  )}
                >
                  <span className="text-sm text-white">{person.firstName} {person.lastName}</span>
                  {person.birthDate && <span className="text-xs text-slate-400">{format(new Date(person.birthDate), 'yyyy')}</span>}
                  ...
                  {person.id === currentStudentId && <Check className="h-4 w-4 text-teal-400 mr-auto" />}
                </button>
              ))}
            </ScrollArea>
          </motion.div>
...
const Step1_FormalInfo = ({ projectData, onUpdate, people, onStudentChange }: Step1FormalInfoProps) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4" dir="rtl">
                <div className={fieldContainerClass}>
                    <label className={labelClass}><User /> מגיש/ה</label>
                    <StudentSelector people={people} currentStudentId={projectData.studentPersonId} onStudentChange={onStudentChange} />
                </div>
                <div className={fieldContainerClass}>
  ...
  const renderStepContent = () => {
    switch (project.currentStep) {
        case 0:
            ...
            />;
        case 1: return <Step1_FormalInfo projectData={project.projectData} onUpdate={handleProjectUpdate} people={people} onStudentChange={handleSelectStudent} />;
...
I have created the single change block as requested.I understand. I have made the targeted fixes to `RootsView.tsx` as you requested.

Here is a summary of the changes:

1.  **RTL Grid Layout**: The fields in the "Official Cover Page" step are now correctly ordered from right to left for a proper RTL layout.
2.  **High-Contrast Dark Inputs**: All input fields and textareas have been updated with the new high-contrast dark theme styling, ensuring they are clearly visible and integrated into the "Cosmic" design.
3.  **Interactive Student Selector**: The static student name display has been replaced with a fully interactive dropdown component, allowing you to change the student for the project directly from the cover page step. The necessary data and functions have been passed down to enable this.
4.  **Component & Import Fixes**: The missing `Loader2` icon has been imported, and the `MotionButton` and `GlassmorphicCard` components have been refactored to resolve underlying React errors. The layout sizing and spacing have also been adjusted as requested.

These fixes address the specific UI, logic, and responsive issues you pointed out.

<changes>
  <description>This change fixes several specific UI and logic bugs in the Roots Project Wizard (RootsView.tsx). It corrects the RTL grid column order for form fields, implements a high-contrast dark theme for all input fields to improve visibility, and replaces a static student name display with a new, fully interactive "StudentSelector" dropdown component. Additionally, it adjusts responsive layout sizing for better screen fit and fixes several underlying component errors, including a missing icon import and incorrect forwardRef implementations.</description>
  <change>
    <file>/src/app/tree/[treeId]/views/RootsView.tsx</file>
